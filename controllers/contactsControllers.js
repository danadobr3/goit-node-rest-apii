...
export const updateContact = async (req, res, next) => {
  try {
    const updatedContact = await contactsService.updateContact(req.params.id, req.body, req.user);
    if (!updatedContact) {
      throw HttpError(404, "Not found");
    }
...
export const updateStatusContact = async (req, res, next) => {
  try {
    const updatedStatus = await contactsService.updateStatusContact(req.params.id, req.body, req.user);
    if (!updatedStatus) {
      throw HttpError(404, "Not found");
    }
    if (Object.keys(req.body).length === 0) {
